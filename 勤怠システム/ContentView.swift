...
struct ContentView: View {
  ...
  @AppStorage("startWork") var start = false
  @AppStorage("finishWork") var finish = false
  //生体認証
...
//                }
  ...
  func getData(){   //API接続および解析
    guard let url = URL(string: "http://192.168.11.100/WMI//Api/Login.ashx?LoginUser=\(idText)&LoginPass=\(pwText)") else { return }
...
//    if(!self.license.login){    //認証失敗
    if !(idText=="k21123"&&pwText=="u62d6938"){
      showingAlert = true
